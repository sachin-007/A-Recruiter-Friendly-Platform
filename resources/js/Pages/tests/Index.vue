<template>
  <div>
    <div class="mb-6 flex items-center justify-between">
      ...
      <router-link to="/tests/create" class="rounded bg-indigo-600 px-4 py-2 text-white">Create Test</router-link>
    </div>
        ...
        <tbody class="divide-y divide-gray-200 bg-white">
          <tr v-for="test in tests.data" :key="test.id">
            ...
            <td class="px-4 py-3 text-right space-x-3">
              <router-link :to="`/tests/${test.id}`" class="text-indigo-600">View</router-link>
              <router-link :to="`/tests/${test.id}/edit`" class="text-indigo-600">Edit</router-link>
            </td>
          </tr>
        </tbody>
      </table>
